...
async function generateSummary(
  ...
  const prompt = `
    ...
    4. עבור כל לינק, תן את המידע הבא:
       - שם הכלי/האתר/המאמר בהדגשה
       - תיאור קצר ומדויק (2-3 משפטים) של מה הלינק מציע ולמה הוא שימושי ליזמים
       - אם זה כלי, ציין את התכונות העיקריות שלו
       - אם זה מאמר או פוסט, ציין את הנושא העיקרי והתובנות המרכזיות
    ...
    - עבור כל לינק, הצג אותו בפורמט הבא:
      *שם הכלי/האתר* - תיאור קצר ומדויק של הכלי והיתרונות שלו ליזמים. הכלי מציע [תכונות עיקריות].
      לינק: [URL]
    - השתמש במקפים (-) בתחילת כל פריט
    - מספר את הקטגוריות בצורה ברורה ויישר אותן לימין
    - הסיכום צריך להיות קל להעתקה והדבקה לקבוצת וואטסאפ
    
    חשוב: עבור כל לינק, נסה להבין את התוכן שלו ולספק מידע אמיתי ושימושי. אל תמציא מידע אם אינך בטוח.
  `;
  ...
  try {
    ...
    if (error instanceof Error && 
      ...
      try {
        ...
        const simplifiedPrompt = `סכם את הלינקים הבאים בקצרה ובצורה מפורטת:
          ...
          עבור כל לינק, תן:
          - שם הכלי/האתר בהדגשה
          - תיאור קצר ומדויק של מה הלינק מציע
          - הלינק עצמו בשורה נפרדת בפורמט: "לינק: [URL]"
